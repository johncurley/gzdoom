...
SDL2DisplayWindow::SDL2DisplayWindow(DisplayWindowHost *windowHost,
                                     bool popupWindow, SDL2DisplayWindow *owner,
                                     RenderAPI renderAPI, double uiscale)
  ...
  if (renderAPI == RenderAPI::Vulkan || renderAPI == RenderAPI::OpenGL ||
      renderAPI == RenderAPI::Metal) {
    Handle.window = SDL_CreateWindow("", SDL_WINDOWPOS_CENTERED,
                                     SDL_WINDOWPOS_CENTERED, 320, 200, flags);
    if (!Handle.window)
      throw std::runtime_error(std::string("Unable to create SDL window:") +
                               SDL_GetError());
  } else {
    int result = SDL_CreateWindowAndRenderer(320, 200, flags, &Handle.window,
                                             &RendererHandle);
    if (result != 0)
      throw std::runtime_error(std::string("Unable to create SDL window:") +
                               SDL_GetError());
...
VkSurfaceKHR SDL2DisplayWindow::CreateVulkanSurface(VkInstance instance) {
  ...
  if (SDL_Vulkan_CreateSurface(Handle.window, instance, &surfaceHandle) !=
      SDL_TRUE)
...
void SDL2DisplayWindow::SetWindowIcon(
    const std::vector<std::shared_ptr<Image>> &images) {
  // To do: call SDL_SetWindowIcon for the highest resolution image (or maybe
  // the first 32x32 or higher? not sure what is best here) To do: when this is
  // upgraded to SDL3, call SDL_AddSurfaceAlternateImage for all the images
}
...
void SDL2DisplayWindow::Show() { SDL_ShowWindow(Handle.window); }
...
bool SDL2DisplayWindow::IsWindowFullscreen() { return isFullscreen; }

void SDL2DisplayWindow::Hide() { SDL_HideWindow(Handle.window); }

void SDL2DisplayWindow::Activate() { SDL_RaiseWindow(Handle.window); }

void SDL2DisplayWindow::ShowCursor(bool enable) { SDL_ShowCursor(enable); }

void SDL2DisplayWindow::LockKeyboard() {
  // Enables raw keyboard scancode events (OnRawKeyboard should be called for
  // keyboard input)
}

void SDL2DisplayWindow::UnlockKeyboard() {
  // Disable raw keyboard scancode events (OnKeyDown/OnKeyUp/OnKeyChar should be
  // called for keyboard input)
}
...
double SDL2DisplayWindow::GetDpiScale() const {
  // SDL2 doesn't really support this properly. SDL_GetDisplayDPI returns the
  // wrong information according to the docs. We currently cheat by asking X11
  // directly. See the SDL2DisplayBackend constructor.
  return UIScale;
}

void SDL2DisplayWindow::PresentBitmap(int width, int height,
                                      const uint32_t *pixels) {
  if (!RendererHandle)
    return;

  if (!BackBufferTexture || BackBufferWidth != width ||
    ...
    BackBufferTexture =
        SDL_CreateTexture(RendererHandle, SDL_PIXELFORMAT_ARGB8888,
...
void SDL2DisplayWindow::RunLoop() {
  ...
  while (!ExitRunLoop) {
    ...
    if (result == 1)
      DispatchEvent(event); // Silently ignore if it fails and pray it doesn't
                            // busy loop, because SDL and Linux utterly sucks!
  }
}

void SDL2DisplayWindow::ExitLoop() { ExitRunLoop = true; }
...
void *SDL2DisplayWindow::StartTimer(int timeoutMilliseconds,
                                    std::function<void()> onTimer) {
  void *execID = (void *)(uintptr_t)++TimerIDs;
  void *id = (void *)(uintptr_t)SDL_AddTimer(
      timeoutMilliseconds, SDL2DisplayWindow::ExecTimer, execID);
...
SDL2DisplayWindow *SDL2DisplayWindow::FindEventWindow(const SDL_Event &event) {
  ...
  case SDL_CONTROLLERBUTTONUP:
    break; // use last event
  case SDL_CONTROLLERBUTTONDOWN:
    break; // use last event
  default:
    if (event.type == PaintEventNumber)
...
void SDL2DisplayWindow::DispatchEvent(const SDL_Event &event) {
  ...
  if (dropJoyDown && event.type == SDL_CONTROLLERBUTTONDOWN)
    return;
  ...
  case SDL_MOUSEMOTION:
    ...
    break;
  case SDL_CONTROLLERBUTTONUP:
    window->OnJoyButtonUp(event.cbutton);
    break;
  case SDL_CONTROLLERBUTTONDOWN:
    window->OnJoyButtonDown(event.cbutton);
    break;
  default:
    if (event.type == PaintEventNumber)
      window->OnPaintEvent();
...
void SDL2DisplayWindow::OnWindowEvent(const SDL_WindowEvent &event) {
  ...
  case SDL_WINDOWEVENT_FOCUS_GAINED:
    fprintf(stderr,
            "ZWidget SDL_WINDOWEVENT_FOCUS_GAINED -> OnWindowActivated()\n");
    WindowHost->OnWindowActivated();
    break;

  case SDL_WINDOWEVENT_FOCUS_LOST:
    fprintf(stderr,
            "ZWidget SDL_WINDOWEVENT_FOCUS_LOST -> OnWindowDeactivated()\n");
...
void SDL2DisplayWindow::OnJoyButtonUp(const SDL_ControllerButtonEvent &event) {
  WindowHost->OnWindowKeyUp(
      GameControllerButtonToInputKey((SDL_GameControllerButton)event.button));
}

void SDL2DisplayWindow::OnJoyButtonDown(
    const SDL_ControllerButtonEvent &event) {
  WindowHost->OnWindowKeyDown(
      GameControllerButtonToInputKey((SDL_GameControllerButton)event.button));
}
...
InputKey
SDL2DisplayWindow::GetMouseButtonKey(const SDL_MouseButtonEvent &event) {
  switch (event.button) {
...
void SDL2DisplayWindow::OnPaintEvent() { WindowHost->OnWindowPaint(); }
...
InputKey SDL2DisplayWindow::ScancodeToInputKey(SDL_Scancode keycode) {
  ...
  case SDL_SCANCODE_PRINTSCREEN:
    return InputKey::Print;
  case SDL_SCANCODE_EXECUTE:
    return InputKey::Execute;
...
InputKey SDL2DisplayWindow::GameControllerButtonToInputKey(
