...
InputKey keycode_to_inputkey(unsigned short keycode) {
  static const std::map<unsigned short, InputKey> keymap = {
      {0, InputKey::A},          {1, InputKey::S},
      {2, InputKey::D},          {3, InputKey::F},
      {4, InputKey::H},          {5, InputKey::G},
      {6, InputKey::Z},          {7, InputKey::X},
      {8, InputKey::C},          {9, InputKey::V},
      {11, InputKey::B},         {12, InputKey::Q},
      {13, InputKey::W},         {14, InputKey::E},
      {15, InputKey::R},         {16, InputKey::Y},
      {17, InputKey::T},         {36, InputKey::Enter},
      {48, InputKey::Tab},       {49, InputKey::Space},
      {51, InputKey::Backspace}, {53, InputKey::Escape},
      {55, InputKey::LCommand}, // Command key
      {56, InputKey::LShift},    {57, InputKey::CapsLock},
      {58, InputKey::Alt}, // Left Alt (Option)
      {59, InputKey::LControl},  {60, InputKey::RShift},
      {61, InputKey::Alt}, // Right Alt (Option)
      {62, InputKey::RControl},  {115, InputKey::Home},
      {116, InputKey::PageUp},   {117, InputKey::Delete},
      {119, InputKey::End},      {121, InputKey::PageDown},
      {122, InputKey::F1},       {120, InputKey::F2},
      {99, InputKey::F3},        {118, InputKey::F4},
      {96, InputKey::F5},        {97, InputKey::F6},
      {98, InputKey::F7},        {100, InputKey::F8},
      {101, InputKey::F9},       {109, InputKey::F10},
      {103, InputKey::F11},      {111, InputKey::F12},
      {123, InputKey::Left},     {124, InputKey::Right},
      {125, InputKey::Down},     {126, InputKey::Up}};
...
static CVReturn DisplayLinkOutputCallback(CVDisplayLinkRef displayLink,
  ...
  return impl->displayLinkOutputCallback(displayLink, inNow, inOutputTime,
                                         flagsIn, flagsOut, displayLinkContext);
}
...
- (id)initWithImpl:(CocoaDisplayWindowImpl *)d {
  ...
  if (self) {
    ...
    // CRITICAL: Don't access self.layer for Bitmap rendering - accessing it
    // creates a layer! Don't create tracking area here - it will be created in
    // updateTrackingAreas
    trackingArea = nil;
  }
  return self;
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  if (impl->renderAPI == RenderAPI::Bitmap ||
      impl->renderAPI == RenderAPI::Unspecified) {
...
- (void)viewDidMoveToWindow {
  if ([self window]) {
    ...
    // Only start displayLink for OpenGL/Metal rendering (continuous refresh
    // needed) Bitmap rendering is event-driven, no continuous refresh needed
...
- (void)mouseMoved:(NSEvent *)theEvent {
  if (impl && impl->windowHost) {
    if (impl->mouseCaptured) {
      impl->windowHost->OnWindowRawMouseMove([theEvent deltaX],
                                             [theEvent deltaY]);
    } else {
      NSPoint p = [theEvent locationInWindow];
      impl->windowHost->OnWindowMouseMove(
          Point(p.x, [self frame].size.height - p.y));
    }
  }
}
...
- (void)mouseDown:(NSEvent *)theEvent {
  if (impl && impl->windowHost) {
    ...
    if ([theEvent buttonNumber] == 0)
      mouseKey = InputKey::LeftMouse;
    else if ([theEvent buttonNumber] == 1)
      mouseKey = InputKey::RightMouse;
    else if ([theEvent buttonNumber] == 2)
      mouseKey = InputKey::MiddleMouse;
...
- (void)rightMouseDown:(NSEvent *)theEvent {
  if (impl && impl->windowHost) {
    NSPoint p = [theEvent locationInWindow];
    impl->windowHost->OnWindowMouseDown(
        Point(p.x, [self frame].size.height - p.y), InputKey::RightMouse);
  }
}
...
- (void)rightMouseUp:(NSEvent *)theEvent {
  if (impl && impl->windowHost) {
    NSPoint p = [theEvent locationInWindow];
    impl->windowHost->OnWindowMouseUp(
        Point(p.x, [self frame].size.height - p.y), InputKey::RightMouse);
  }
}
...
- (void)scrollWheel:(NSEvent *)theEvent {
  if (impl && impl->windowHost) {
    ...
    if ([theEvent deltaY] > 0)
      impl->windowHost->OnWindowMouseWheel(
          Point(p.x, [self frame].size.height - p.y), InputKey::MouseWheelUp);
    else if ([theEvent deltaY] < 0)
      impl->windowHost->OnWindowMouseWheel(
          Point(p.x, [self frame].size.height - p.y), InputKey::MouseWheelDown);
  }
}
...
- (void)flagsChanged:(NSEvent *)theEvent {
  if (impl && impl->windowHost) {
    bool shiftPressed =
        ([theEvent modifierFlags] & NSEventModifierFlagShift) != 0;
    bool ctrlPressed =
        ([theEvent modifierFlags] & NSEventModifierFlagControl) != 0;
    bool altPressed =
        ([theEvent modifierFlags] & NSEventModifierFlagOption) != 0;
    bool commandPressed =
    ...
    if (impl->keyState[InputKey::LShift] != shiftPressed) {
      ...
      if (shiftPressed)
        impl->windowHost->OnWindowKeyDown(InputKey::LShift);
      else
        impl->windowHost->OnWindowKeyUp(InputKey::LShift);
    }
    if (impl->keyState[InputKey::RShift] != shiftPressed) // Also update RShift
    {
      impl->keyState[InputKey::RShift] = shiftPressed;
      if (shiftPressed)
        impl->windowHost->OnWindowKeyDown(InputKey::RShift);
      else
        impl->windowHost->OnWindowKeyUp(InputKey::RShift);
    }
    ...
    if (impl->keyState[InputKey::LControl] != ctrlPressed) {
      ...
      if (ctrlPressed)
        impl->windowHost->OnWindowKeyDown(InputKey::LControl);
      else
        impl->windowHost->OnWindowKeyUp(InputKey::LControl);
    }
    if (impl->keyState[InputKey::RControl] !=
        ctrlPressed) // Also update RControl
    {
      impl->keyState[InputKey::RControl] = ctrlPressed;
      if (ctrlPressed)
        impl->windowHost->OnWindowKeyDown(InputKey::RControl);
      else
        impl->windowHost->OnWindowKeyUp(InputKey::RControl);
    }
    ...
    if (impl->keyState[InputKey::LCommand] != commandPressed) {
      ...
      if (commandPressed)
        impl->windowHost->OnWindowKeyDown(InputKey::LCommand);
      else
        impl->windowHost->OnWindowKeyUp(InputKey::LCommand);
    }
  }
}
@end
...
- (void)windowDidBecomeKey:(NSNotification *)notification {
  fprintf(stderr, "ZWidget windowDidBecomeKey -> OnWindowActivated()\n");
  if (impl && impl->windowHost) {
    impl->windowHost->OnWindowActivated();
  }
}

- (void)windowDidResignKey:(NSNotification *)notification {
  fprintf(stderr, "ZWidget windowDidResignKey -> OnWindowDeactivated()\n");
  if (impl && impl->windowHost) {
    impl->windowHost->OnWindowDeactivated();
  }
}
...
CVReturn CocoaDisplayWindowImpl::displayLinkOutputCallback(
...
CocoaDisplayWindow::CocoaDisplayWindow(DisplayWindowHost *windowHost,
                                       bool popupWindow, DisplayWindow *owner,
                                       RenderAPI renderAPI) {
...
void CocoaDisplayWindow::SetWindowIcon(
  ...
  if (impl->window && !images.empty()) {
    ...
    CGContextRef context = CGBitmapContextCreate(
        iconImage->GetData(), iconImage->GetWidth(), iconImage->GetHeight(), 8,
        iconImage->GetWidth() * 4, colorSpace,
...
void CocoaDisplayWindow::SetWindowFrame(const Rect &box) {
  if (impl->window) {
    NSRect frame = NSMakeRect(
        box.x, [[NSScreen mainScreen] frame].size.height - box.y - box.height,
...
void CocoaDisplayWindow::CaptureMouse() { impl->mouseCaptured = true; }

void CocoaDisplayWindow::ReleaseMouseCapture() { impl->mouseCaptured = false; }
...
Rect CocoaDisplayWindow::GetWindowFrame() const {
  if (impl->window) {
    ...
    return Rect(frame.origin.x,
                [[NSScreen mainScreen] frame].size.height - frame.origin.y -
                    frame.size.height,
...
Point CocoaDisplayWindow::MapFromGlobal(const Point &pos) const {
  if (impl->window) {
    NSPoint globalPoint =
        NSMakePoint(pos.x, [[NSScreen mainScreen] frame].size.height - pos.y);
    NSPoint windowPoint = [impl->window convertPointFromScreen:globalPoint];
    NSPoint viewPoint = [[impl->window contentView] convertPoint:windowPoint
                                                        fromView:nil];
    return Point(viewPoint.x,
                 [[impl->window contentView] frame].size.height - viewPoint.y);
  }
  return {};
}
...
Point CocoaDisplayWindow::MapToGlobal(const Point &pos) const {
  if (impl->window) {
    NSPoint viewPoint = NSMakePoint(
        pos.x, [[impl->window contentView] frame].size.height - pos.y);
    NSPoint windowPoint = [[impl->window contentView] convertPoint:viewPoint
                                                            toView:nil];
    NSPoint globalPoint = [impl->window convertPointToScreen:windowPoint];
    return Point(globalPoint.x,
                 [[NSScreen mainScreen] frame].size.height - globalPoint.y);
  }
  return {};
}
...
void CocoaDisplayWindow::PresentBitmap(int width, int height,
...
VkSurfaceKHR CocoaDisplayWindow::CreateVulkanSurface(VkInstance instance) {
  if (impl->window && impl->metalLayer) {
    ...
    if (!vkCreateMetalSurfaceEXT) {
      ...
      if (vulkanLib) {
        vkCreateMetalSurfaceEXT = (PFN_vkCreateMetalSurfaceEXT)dlsym(
            vulkanLib, "vkCreateMetalSurfaceEXT");
      }
    }
    ...
    if (vkCreateMetalSurfaceEXT) {
      ...
      if (err != VK_SUCCESS)
        throw std::runtime_error(
            "Could not create vulkan surface: vkCreateMetalSurfaceEXT failed");
      return surface;
    }
  }
