...
Session-by-session working notes for the Metal renderer and compute work,
2026-06 to 2026-08, oldest first. **This is an archive, not a guide** -- read
`AGENTS.md` for current state and `CONTRIBUTING.md` for how to work here.

It is kept because it is load-bearing in one specific way: it records what was
tried and *disproved*. Several hypotheses in here were re-investigated more than
once before anyone wrote down that they were dead. If you are about to chase
something in the Metal renderer, grep this file for it first -- the answer may
already be a measurement rather than a guess.

Entries are verbatim. Where a later entry corrects or retracts an earlier one,
both are kept; the retraction says so explicitly.

---

## Roadmap as it stood at the end of the log

Kept verbatim for the record. Priorities and their DONE/CLOSED
annotations reflect 2026-07/08; the live roadmap is
`docs/engine-modernization.md` and current state is `AGENTS.md`.
